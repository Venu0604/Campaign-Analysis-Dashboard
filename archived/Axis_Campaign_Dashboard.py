...
# Streamlit Config
# -------------------------
st.set_page_config(page_title="Axis Bank Campaign Dashboard", layout="wide", page_icon="🏦")
...
st.markdown('<div class="main-header">🏦 Axis Bank Campaign Analytics</div>', unsafe_allow_html=True)
...
# -------------------------
sheet_url = "https://docs.google.com/spreadsheets/d/184yquIAWt0XyQEYhI3yv0djg9f6pUtZS7TZ4Un7NLXI/export?format=csv&gid=526829508"
...
if mis_file and df_identifiers is not None:
    try:
        ...
        # Find the CROSSCELLCODE column (case insensitive)
        crosscell_col = None
        for col in df_mis.columns:
            if 'crosscell' in col.lower():
                crosscell_col = col
                break

        if crosscell_col is None:
            st.error("❌ CROSSCELLCODE column not found in MIS file. Please check the file structure.")
            st.stop()

        # Find FINAL STATUS column
        status_col = None
        for col in df_mis.columns:
            if 'final' in col.lower() and 'status' in col.lower():
                status_col = col
                break

        if status_col is None:
            st.warning("⚠️ FINAL STATUS column not found. Using available status columns.")
        ...
        # Find IPA STATUS column
        ipa_status_col = None
        for col in df_mis.columns:
            if 'ipa' in col.lower() and 'status' in col.lower():
                ipa_status_col = col
                break
        ...
        for _, row in df_identifiers.iterrows():
            ...
            # Filter MIS data where CROSSCELLCODE contains the identifier
            mask = df_mis[crosscell_col].astype(str).str.contains(identifier, case=False, na=False)
            ...
            # Status analysis based on FINAL STATUS column
            # Card Out = "Approved"
            # Declined = "Declined"
            ...
            if status_col:
                ...
                # Card Out (Approved status)
                card_out = (status_values.str.upper() == 'APPROVED').sum()

                # Declined
                declined = (status_values.str.upper() == 'DECLINED').sum()

                # In Progress (everything except Approved and Declined)
            ...
            # IPA Status analysis
            # Count IPA approved from IPA Status column
            if ipa_status_col:
                ipa_values = df_filtered_mis[ipa_status_col].astype(str).str.strip()
                # IPA Approved includes: "Approved", "IPA Approved", "Underwriter Approved"
                ipa_approved = (
                    (ipa_values.str.upper() == 'APPROVED') |
                    (ipa_values.str.upper() == 'IPA APPROVED') |
                    (ipa_values.str.upper() == 'UNDERWRITER APPROVED') |
                    (ipa_values.str.upper() == 'RCU') |
                    (ipa_values.str.upper() == 'UW')
                ).sum()
            else:
                # If no IPA status column, estimate from FINAL STATUS
                ipa_approved = card_out  # Assume all card out were IPA approved
        ...
        with tab4:
            ...
            st.write("This tab shows the detailed MIS records that matched each campaign identifier via CROSSCELLCODE.")
            ...
            if not df_matched_mis_all.empty:
                ...
                with mis_col3:
                    if status_col and status_col in df_matched_display.columns:
                        completed_count = df_matched_display[status_col].astype(str).str.contains('COMPLETE|SUCCESS|ISSUE', case=False, na=False).sum()
                        st.metric("Completed/Success", f"{completed_count:,}")
                with mis_col4:
                    if status_col and status_col in df_matched_display.columns:
                        pending_count = df_matched_display[status_col].astype(str).str.contains('PROGRESS|PENDING', case=False, na=False).sum()
                        st.metric("In Progress", f"{pending_count:,}")
                ...
                st.download_button(
                    ...
                    data=csv_matched,
                    file_name=f"Axis_Matched_MIS_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv",
                )
            else:
        ...
        with tab5:
            ...
            with export_col1:
                ...
                st.download_button(
                    ...
                    data=output,
                    file_name=f"Axis_Campaign_Report_{datetime.now().strftime('%Y%m%d')}.xlsx",
            ...
            with export_col2:
                ...
                st.download_button(
                    ...
                    data=csv_data,
                    file_name=f"Axis_Campaign_Data_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv",
                )
    ...
    with col_welcome2:
        st.markdown("""
            <div style='text-align: center; padding: 3rem;'>
                <h2 style='color: #e2e8f0;'>👋 Welcome to Axis Bank Campaign Analytics</h2>
                ...
                <div style='margin-top: 2rem; padding: 2rem;
                    ...
                    <ul style='list-style: none; padding: 0; text-align: left;'>
                        ...
                        <li style='padding: 0.5rem 0;'>🔍 CROSSCELLCODE matching</li>
                    </ul>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
st.markdown("""
    <div style='text-align: center; color: #94a3b8; padding: 1rem;'>
        <p>Built with ❤️ for Axis Bank | © 2025 Campaign Analytics Dashboard</p>
    </div>
""", unsafe_allow_html=True)
