...
if mis_file and df_identifiers is not None:
    try:
        ...
        for _, row in df_identifiers.iterrows():
            ...
            # Count Card Out (where OPS Status = "Card Setup") - case insensitive
            card_out = 0
            if ops_status_col:
                card_out = len(df_filtered[df_filtered[ops_status_col].astype(str).str.strip().str.lower() == "card setup"])
        ...
        with tab3:
            ...
            # Sub-tabs for different summaries
            summary_tab1, summary_tab2 = st.tabs(["📢 Channel Summary", "🎯 Identifier Summary"])
            ...
            with summary_tab1:
                ...
                # Calculate metrics
                channel_summary['Cost per Application'] = channel_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Applications'] if row['Applications'] > 0 else 0, axis=1
                )
                channel_summary['Cost per IPA approved'] = channel_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Approved'] if row['Approved'] > 0 else 0, axis=1
                )
                channel_summary['Cost per Card Out'] = channel_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Card Out'] if row['Card Out'] > 0 else 0, axis=1
                )
                
                # Reorder columns
                channel_summary = channel_summary[[
                    'Channel', 'Applications', 'Card Out', 'In Progress', 'Declined', 
                    'Approved', 'Delivered', 'Cost per unit (₹)', 'Total cost (₹)',
                    'Cost per Application', 'Cost per IPA approved', 'Cost per Card Out'
                ]]
            ...
            with summary_tab2:
                st.markdown("#### Identifier-wise Summary (Campaign-wise)")
                
                # Create identifier summary
                identifier_summary = df_filtered.groupby('Campaign name').agg({
                    'Applications': 'sum',
                    'AIP Decline': 'sum',
                    'Received For VKYC': 'sum',
                    'Touch Free KYC Pending': 'sum',
                    'Received For Biometric': 'sum',
                    'Declined': 'sum',
                    'Approved': 'sum',
                    'Card Out': 'sum',
                    'In Progress': 'sum',
                ...
                # Calculate metrics
                identifier_summary['CTR (%)'] = identifier_summary.apply(
                    lambda row: round((row['Clicks'] / row['Delivered'] * 100), 2) if row['Delivered'] > 0 else 0, axis=1
                )
                identifier_summary['Cost per Application'] = identifier_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Applications'] if row['Applications'] > 0 else 0, axis=1
                )
                identifier_summary['Cost per IPA approved'] = identifier_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Approved'] if row['Approved'] > 0 else 0, axis=1
                )
                identifier_summary['Cost per Card Out'] = identifier_summary.apply(
                    lambda row: row['Total cost (₹)'] / row['Card Out'] if row['Card Out'] > 0 else 0, axis=1
                )
                
                # Reorder columns
                identifier_summary = identifier_summary[[
                    'Date', 'Campaign name', 'Applications', 'AIP Decline', 'Received For VKYC',
                    'Touch Free KYC Pending', 'Received For Biometric', 'Declined', 'Approved',
                    'Card Out', 'In Progress', 'Delivered', 'Clicks', 'CTR (%)',
                    'Total cost (₹)', 'Cost per Application', 'Cost per IPA approved',
                    'Cost per Card Out', 'Source', 'Channel'
                ]]
                
                # Sort by date descending
                identifier_summary = identifier_summary.sort_values('Date', ascending=False)
                
                st.dataframe(
                    identifier_summary.style.format({
                        'Date': lambda x: x.strftime('%Y-%m-%d') if pd.notna(x) else '',
                        'Applications': '{:.0f}',
                        'AIP Decline': '{:.0f}',
                        'Received For VKYC': '{:.0f}',
                        'Touch Free KYC Pending': '{:.0f}',
                        'Received For Biometric': '{:.0f}',
                        'Declined': '{:.0f}',
                        'Approved': '{:.0f}',
                        'Card Out': '{:.0f}',
                        'In Progress': '{:.0f}',
        ...
        with tab4:
            ...
            search_term = st.text_input("🔍 Search", placeholder="Enter campaign name...")
        ...
        with tab5:
            ...
            with export_col1:
                ...
                st.write("All metrics and summaries")
                
                # Prepare matched MIS data
                matched_mis_data = []
                for _, row in df_identifiers.iterrows():
                    identifier = str(row.get("Identifiers", "")).strip()
                    mask = df_mis[campaign_source_col].astype(str).str.contains(identifier, case=False, na=False)
                    df_matched = df_mis[mask].copy()
                    if len(df_matched) > 0:
                        df_matched['Matched Identifier'] = identifier
                        matched_mis_data.append(df_matched)

                # Combine all matched MIS data
                if matched_mis_data:
                    df_matched_mis = pd.concat(matched_mis_data, ignore_index=True)
                else:
                    df_matched_mis = pd.DataFrame()

                output = BytesIO()
                with pd.ExcelWriter(output, engine="openpyxl") as writer:
                    # Campaign Data Sheet
                    df_filtered.to_excel(writer, sheet_name="Campaign Data", index=False)

                    # Channel Summary Sheet
                    channel_summary.to_excel(writer, sheet_name="Channel Summary", index=False)

                    # Identifier Summary Sheet
                    identifier_summary.to_excel(writer, sheet_name="Identifier Summary", index=False)

                    # Matched MIS Data Sheet
                    if not df_matched_mis.empty:
                        df_matched_mis.to_excel(writer, sheet_name="Matched MIS", index=False)

                    # Metrics Sheet
                    metrics_df = pd.DataFrame({
                        'Metric': ['Total Applications', 'Total Cost', 'Approved',
                                  'Cards Issued', 'Avg CPA', 'Avg CTR'],
                        'Value': [total_apps, f"₹{total_cost:.2f}", total_approved,
                ...
                st.download_button(
                    ...
                    data=output,
                    file_name=f"RBL_Campaign_Report_{datetime.now().strftime('%Y%m%d')}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
            
            with export_col2:
                st.markdown("#### 📄 CSV Export")
                st.write("Download filtered data in CSV format.")
                
                csv_data = df_filtered.to_csv(index=False)
                st.download_button(
                    label="⬇️ Download CSV",
                    data=csv_data,
                    file_name=f"RBL_Campaign_Data_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv",
                )
            ...
            <div style='text-align: center; padding: 3rem;'>
                ...
                <div style='margin-top: 2rem; padding: 2rem; 
                    ...
                    <ul style='list-style: none; padding: 0; text-align: left;'>
                        ...
                        <li style='padding: 0.5rem 0;'>📥 Excel & CSV exports</li>
                    </ul>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
st.markdown("""
    <div style='text-align: center; color: #94a3b8; padding: 1rem;'>
        <p>Built with ❤️ for RBL Bank | © 2025 Campaign Analytics</p>
    </div>
""", unsafe_allow_html=True)